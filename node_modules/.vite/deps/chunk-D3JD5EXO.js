...
function useStateChangeDetection(parameters) {
  ...
  const internalPreviousStateRef = React3.useRef(initialState);
  React3.useEffect(() => {
...
function useControllableReducer(parameters) {
  const lastActionRef = React3.useRef(null);
  const {
    reducer,
    initialState,
  ...
  const controlledPropsRef = React3.useRef(controlledProps);
  if (true) {
    React3.useEffect(() => {
      Object.keys(controlledProps).forEach((key) => {
  ...
  const reducerWithControlledState = React3.useCallback((state, action) => {
    lastActionRef.current = action;
    const controlledState = getControlledState(state, controlledProps);
    const newState = reducer(controlledState, action);
    return newState;
  }, [controlledProps, reducer]);
  const [nextState, dispatch] = React3.useReducer(reducerWithControlledState, initialState);
  const dispatchWithContext = React3.useCallback((action) => {
...
function useList(params) {
  ...
  const handleHighlightChange = React5.useCallback((event, value, reason) => {
  ...
  const stateComparers = React5.useMemo(() => ({
    highlightedValue: itemComparer,
    selectedValues: (valuesArray1, valuesArray2) => areArraysEqual(valuesArray1, valuesArray2, itemComparer)
  }), [itemComparer]);
  const handleStateChange = React5.useCallback((event, field, value, reason, state2) => {
  ...
  const listActionContext = React5.useMemo(() => {
    return {
      disabledItemsFocusable,
      disableListWrap,
      focusManagement,
  ...
  const actionContext = React5.useMemo(() => _extends({}, reducerActionContext, listActionContext), [reducerActionContext, listActionContext]);
  ...
  const previousItems = React5.useRef([]);
  React5.useEffect(() => {
  ...
  const getItemState = React5.useCallback((item) => {
...
function useListItem(parameters) {
  ...
  const listContext = React6.useContext(ListContext);
  if (!listContext) {
  ...
  const createHandleClick = React6.useCallback((externalHandlers) => (event) => {
  ...
  const createHandlePointerOver = React6.useCallback((externalHandlers) => (event) => {
...
var CompoundComponentContext = React7.createContext(null);
if (true) {
...
function useCompoundParent() {
  const [subitems, setSubitems] = React7.useState(/* @__PURE__ */ new Map());
  const subitemKeys = React7.useRef(/* @__PURE__ */ new Set());
  const deregisterItem = React7.useCallback(function deregisterItem2(id) {
  ...
  const registerItem = React7.useCallback(function registerItem2(id, item) {
  ...
  const sortedSubitems = React7.useMemo(() => sortSubitems(subitems), [subitems]);
  const getItemIndex = React7.useCallback(function getItemIndex2(id) {
...
function useCompoundItem(id, itemMetadata) {
  const context = React8.useContext(CompoundComponentContext);
  if (context === null) {
  ...
  const [registeredId, setRegisteredId] = React8.useState(typeof id === "function" ? void 0 : id);
  useEnhancedEffect_default(() => {
    const {
      id: returnedId,
      deregister
...
function useOption(params) {
  ...
  const optionRef = React9.useRef(null);
  const selectOption = React9.useMemo(() => ({
    disabled,
    label,
    value,
    ref: optionRef,
...
function useOptionContextStabilizer(value) {
  const listContext = React10.useContext(ListContext);
  if (!listContext) {
  ...
  const localGetItemState = React10.useCallback((itemValue) => {
    if (itemValue !== value) {
  ...
  const localContextValue = React10.useMemo(() => ({
    dispatch,
...
//# sourceMappingURL=chunk-D3JD5EXO.js.map
