...
function useList(params) {
  ...
  const handleHighlightChange = React4.useCallback((event, value, reason) => {
  ...
  const stateComparers = React4.useMemo(() => ({
    highlightedValue: itemComparer,
    selectedValues: (valuesArray1, valuesArray2) => areArraysEqual(valuesArray1, valuesArray2, itemComparer)
  }), [itemComparer]);
  const handleStateChange = React4.useCallback((event, field, value, reason, state2) => {
  ...
  const listActionContext = React4.useMemo(() => {
    return {
      disabledItemsFocusable,
      disableListWrap,
      focusManagement,
  ...
  const actionContext = React4.useMemo(() => _extends({}, reducerActionContext, listActionContext), [reducerActionContext, listActionContext]);
  ...
  const previousItems = React4.useRef([]);
  React4.useEffect(() => {
  ...
  const getItemState = React4.useCallback((item) => {
...
var CompoundComponentContext = React6.createContext(null);
if (true) {
...
function useCompoundParent() {
  const [subitems, setSubitems] = React6.useState(/* @__PURE__ */ new Map());
  const subitemKeys = React6.useRef(/* @__PURE__ */ new Set());
  const deregisterItem = React6.useCallback(function deregisterItem2(id) {
  ...
  const registerItem = React6.useCallback(function registerItem2(id, item) {
  ...
  const sortedSubitems = React6.useMemo(() => sortSubitems(subitems), [subitems]);
  const getItemIndex = React6.useCallback(function getItemIndex2(id) {
...
function useCompoundItem(id, itemMetadata) {
  const context = React7.useContext(CompoundComponentContext);
  if (context === null) {
  ...
  const [registeredId, setRegisteredId] = React7.useState(typeof id === "function" ? void 0 : id);
  useEnhancedEffect_default(() => {
    const {
      id: returnedId,
      deregister
...
//# sourceMappingURL=chunk-RMMAFBPX.js.map
